...
function OpenSeadragonViewer(props) {
    ...
    if (image && viewer) {
      ...
      if (anno) {
        ...
        annotorious.on("createSelection", function (annotation) {
          ...
          let coordinates = annotation.target.selector.value
            .substring(11)
            .split(",");
          coordinates[0] = parseInt(coordinates[0], 10);
          coordinates[1] = parseInt(coordinates[1], 10);
          coordinates[2] =
            parseInt(coordinates[0], 10) + parseInt(coordinates[2], 10);
          coordinates[3] =
            parseInt(coordinates[1], 10) + parseInt(coordinates[3], 10);
  ...
  }, [image, viewer, configAnno, setCoordinates, setDisplayButton]);
  ...
  return (
    <>
      <p className="hintAnnotorious">
        To select an area, hold the SHIFT key while clicking and dragging the
        mouse
      </p>
      <div
        id="openSeaDragon"
      ...
      {image === null ? (
        <p className="selectImageErrorMessage">
